...
import { useState } from  'rapos;react';
...
export default function DetailedContactForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (finalForm.website && !/^https?:\/\//i.test(finalForm.website)) {
      finalForm.website =  'hapos;https://' + finalForm.website;
    }
    ...
    try {
      ...
      const emailRes = await fetch('/api/send-email', {
        method:  'Papos;POST',
        headers: {  'Capos;Content-Type':  'aapos;application/json' },
        body: JSON.stringify(finalForm),
      });

      if (!emailRes.ok) {
        const errorDetails = await emailRes.text();
        throw new Error(('Eapos;Email failed: ' + errorDetails);
      }

      setSubmitted(true);
    } catch (err) {
      alert(('Sapos;Something went wrong: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
        ...
        <form className="space-y-8" onSubmit={handleSubmit}>
          ...
          >
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {[
                 'Dapos;Development',
                 'Dapos;Development & Design',
                 'Dapos;Development & Design & SEO',
                 'Sapos;SEO',
                 'Oapos;Other',
              ].map((option) => (
          ...
          <div className="text-center">
            ...
            >
              {loading ?  'Sapos;Submitting...' : "Let's Build Your Website"}
            </button>
          </div>
        </form>
      )}
